...
# Calculate the correlation matrix
correlation_matrix3 = cardio_df.corr()
...
# Initialize empty strings to store text box input
if "current_display" not in st.session_state:
    st.session_state.current_display = ""

if "fixes_needed" not in st.session_state:
    st.session_state.fixes_needed = ""
...
# -------------------------------------------------------------------------------------------------------
    ...
    # Add the current status update to the list, including code changes
    status_update = f"{current_datetime} - Status update"
    
    # Display the text box to input what is currently displayed
    st.subheader("Enter What Is Currently Displayed In The Streamlit Web App:")
    current_display = st.text_area("Type here...", key="current_display")
    ...
    if st.button("Save Status"):
        # Include the current_display content in the status update if not empty
        if current_display:
